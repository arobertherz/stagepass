(function () {
  ...
  function clean(d: string | null): string | null {
  ...
  // Re-check session status after parameter processing
  let sv = localStorage.getItem(K);
  // Validate stored domain
  if (sv && !isValidDomain(sv)) {
    localStorage.removeItem(K);
    sv = null;
  }
  // Log connection if session exists but parameter was not just processed
  ...
  function swapEl(sel: string, pathAttr: string, srcAttr: string, type: 'script' | 'link', skipAttrs: string[]) {
    ...
    els.forEach((el: any) => {
      ...
      if (!ps) return;
      // If no localPath is set, extract filename from production URL
      if (!lp && ps && swap && dom) {
        try {
      ...
      if (url) {
        ...
        if (type === 'script') {
          ...
          // Copy all attributes from original script (including async/defer if present)
          Array.from(el.attributes).forEach((a) => {
            const attr = a as Attr;
            if (!skipAttrs.includes(attr.name)) n.setAttribute(attr.name, attr.value);
          });
          // If neither async nor defer is set, use defer as default
